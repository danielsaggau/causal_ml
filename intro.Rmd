...
While various fields have tried to provide parametric causal models, there are various challenges.
One example is the consistency of causal models based on observational data.
The consistency of observational data is more often than not violated, making the correct parametric estimation challenging. 
Henceforth, models with non-parametric specification are gaining prominence within scholarshop.
...
Because we ignore the interventional distribution, we cannot create counterfactuals based on bayesian causal networks. 
...
Compared to the BCN, the SCM specifies the intervention distribution for data outside of our observations and also specifies relationships based on equations.
Structural Causal Models (SCM) are a nonparametric modification of structural equations models with some extensions.
...
Note that these equations have not always had a concise mathematical notation. 
Intially there was no sign to express the assignment equation and people used the '=' and would e.g. write A=B.
But treating an equation as a algebraic equation, an equation that is symmetric makes no sense in the causality context because it would imply that B=A.
Henceforth, the initial = sign was replaced with thw ':=' which is asymmetric (Pearl, 2009).
