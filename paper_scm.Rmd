...
Additionally, this paper provides an overview of various other statistical and causal methods within the grand scheme of pearls causal hierarchy.
...
Subsection 2.1 introduces a baseline example for both a simple set of assignments and the underlying causal graph.
Subsection 2.2 provides further context to assumptions in causal modelling. 
...
Section 5 studies the causal hierachy.
Section 6 suggests instructions on when and when not to employ a structural causal model.
...
As mentioned in the definition for SCMs, we would mathematical notate this independence as follows: $N_{F} \perp \!\!\! \perp N_{C}$ 
...
The graphical approach to recover a graph from an observational dataset requires two assumptions namely jointly independent noise terms and faithfulness. 
...
The third condition is an un-testable assumption.
